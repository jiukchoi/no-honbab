    ...
    <View style={styles.mainView}>
      ...
      <View style={{paddingLeft: 20}}>
        <View style={{flexDirection: 'row', marginBottom: 8, alignItems: 'center', justifyContent: 'space-between', width: 320}}>
          ...
          <TouchableOpacity onPress={() => setIsState((prev) => ({...isState, time: {...isState.time, window: !prev.time.window}}))}>
        ...
        <View style={{flexDirection: 'row', marginBottom: 8, alignItems: 'center', justifyContent: 'space-between', width: 320}}>
          ...
          <TouchableOpacity onPress={() => setIsState((prev) => ({...isState, place: !prev.place}))}>
            <Image source={require('../../../../../public/meetings/arrow-red.png')} />
          </TouchableOpacity>
        </View>
        {isState.time.window && (
          <ScrollView>
            <View style={{flexDirection: 'row'}}>
              <View style={{marginTop: 20, marginRight: 20}}>
                <TouchableOpacity onPress={() => setIsState(prev => ({...isState, time: {...isState.time, pickerMonth: !prev.time.pickerMonth}}))} style={{width: 50, padding: 5, borderRadius: 12, backgroundColor: 'blue', alignItems: 'center'}}>
                  <Text style={{color: 'white', fontWeight: 'bold'}}>{`${input.time.month}월`}</Text>
                </TouchableOpacity>
              </View>
              <View style={{marginTop: 20, marginRight: 20}}>
                <TouchableOpacity onPress={() => setIsState(prev => ({...isState, time: {...isState.time, pickerDate: !prev.time.pickerDate}}))} style={{width: 50, padding: 5, borderRadius: 12, backgroundColor: 'red', alignItems: 'center'}}>
                  <Text style={{color: 'white', fontWeight: 'bold'}}>{`${input.time.date}일`}</Text>
                </TouchableOpacity>
              </View>
              <View style={{marginTop: 20, marginRight: 20}}>
                <TouchableOpacity onPress={() => setIsState(prev => ({...isState, time: {...isState.time, pickerStartTime: !prev.time.pickerStartTime}}))} style={{width: 80, padding: 5, borderRadius: 12, backgroundColor: 'orange', alignItems: 'center'}}>
                  <Text style={{color: 'white', fontWeight: 'bold'}}>{`${input.time.startTime.hours}:${input.time.startTime.mins}`}</Text>
                </TouchableOpacity>
              </View>
              <View style={{marginTop: 20, marginRight: 20}}>
                <TouchableOpacity onPress={() => setIsState(prev => ({...isState, time: {...isState.time, pickerEndTime: !prev.time.pickerEndTime}}))} style={{width: 80, padding: 5, borderRadius: 12, backgroundColor: 'pink', alignItems: 'center'}}>
                  <Text style={{color: 'white', fontWeight: 'bold'}}>{`${input.time.endTime.hours}:${input.time.endTime.mins}`}</Text>
                </TouchableOpacity>
              </View>
            </View>
        ...
        {!isState.time.window && !isState.place && (
          <View>
            ...
            <TouchableOpacity onPress={() => {navigation.navigate('게시물 목록')}} disabled={!isState.create} style={[styles.button, isState.create ? {backgroundColor: '#E24E4A'} : {backgroundColor: '#bdbdbd'} ]}>
